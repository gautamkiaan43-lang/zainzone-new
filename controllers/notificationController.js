...
exports.getAll = async (req, res) => {
    try {
        ...
        const normalizedRole = (role === 'saas_client' || role === 'business_client' || role === 'client') ? 'admin' : role;
        const companyId = req.companyScope;

        let query = `SELECT * FROM notifications WHERE (user_id = ? OR (role_target IN (?, ?) AND (company_id = ? OR company_id IS NULL)))`;
        const params = [userId, role, normalizedRole, companyId];
...
exports.getUnreadCount = async (req, res) => {
    try {
        ...
        let query = `SELECT COUNT(*) as count FROM notifications WHERE is_read = FALSE AND (user_id = ? OR (role_target IN (?, ?) AND (company_id = ? OR company_id IS NULL)))`;
        const params = [userId, role, normalizedRole, companyId];
...
exports.markAllRead = async (req, res) => {
    try {
        ...
        if (role === 'super_admin') {
            ...
            await db.query(
                `UPDATE notifications SET is_read = TRUE WHERE (user_id = ? OR (role_target IN (?, ?) AND (company_id = ? OR company_id IS NULL)))`,
                [userId, role, normalizedRole, companyId]
            );
        }

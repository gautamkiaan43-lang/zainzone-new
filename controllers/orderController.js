...
function extractDeliveryModeFromNotes(notes) {
    if (!notes) return 'Road';
    const match = String(notes).match(/delivery_mode\s*:\s*([^;\]\n]+)/i);
    const mode = match ? match[1].trim().toLowerCase() : '';
    if (mode === 'sea' || mode === 'boat' || mode === 'marine') return 'Sea';
    if (mode === 'air' || mode === 'flight' || mode === 'plane') return 'Air';
    return 'Road';
}
        ...
        // Admins also see DEFAULT_COMPANY_ID (HQ) orders so personal/marketplace checkout is visible alongside tenant work.
            ...
            if (stageRoles.includes(role)) {
                ...
                if (role === 'operation') {
                    ...
                    whereClause += ' AND (o.current_stage = \'concierge\' OR o.status = \'concierge\' OR o.created_by = ?)';
                    params.push(req.user.id);
                } else {
...
exports.create = async (req, res) => {
    try {
        ...
        if (initialStage === 'logistics') {
            const transportMode = extractDeliveryModeFromNotes(notesMerged);
            await db.query(
                `INSERT INTO deliveries (company_id, order_id, mission_type, status, pickup_location, drop_location, package_details, delivery_date, mode) 
                 VALUES (?, ?, 'Delivery', 'pending', ?, ?, ?, ?, ?)`,
                [companyId, orderId, pickup_location || pickupLocation || 'Warehouse / HQ', finalDeliveryAddress || 'Customer Address', JSON.stringify(parsedItems), toMysqlDateOnly(orderDateVal), transportMode]
            );
...
exports.update = async (req, res) => {
    try {
        ...
        const [[existingOrder]] = await db.query('SELECT notes FROM orders WHERE id = ?', [req.params.id]);
        if (!existingOrder) return res.status(404).json({ success: false, message: 'Order not found' });
        
        let existingNotes = req.body.notes !== undefined ? req.body.notes : (existingOrder.notes || '');

        // Translate deliveryType (frontend) into a meta block stored in the notes column.
        if (req.body.deliveryType) {
            const newMode = String(req.body.deliveryType).trim();
            if (existingNotes && /delivery_mode:\s*[a-zA-Z]+/i.test(existingNotes)) {
                existingNotes = existingNotes.replace(/delivery_mode:\s*[a-zA-Z]+/i, `delivery_mode:${newMode}`);
            } else {
                const block = `[request_meta] delivery_mode:${newMode}`;
                existingNotes = existingNotes ? `${existingNotes}\n\n${block}` : block;
            }
            req.body.notes = existingNotes;
        }
    ...
    try {
        ...
        if (stage === 'logistics') {
            ...
            if (!existingDel.length) {
                ...
                const notesMerged = [order.notes, order.order_notes].filter(Boolean).join('; ');
                const transportMode = extractDeliveryModeFromNotes(notesMerged);
                ...
                try {
                    await db.query(
                        `INSERT INTO deliveries (company_id, order_id, mission_type, status, pickup_location, drop_location, package_details, delivery_date, delivery_instructions, delivery_fee, mode)
                         VALUES (?, ?, 'Delivery', 'pending', ?, ?, ?, ?, ?, ?, ?)`,
                        [
                            order.company_id,
                            id,
                    ...
                    if (insErr.code === 'ER_DUP_ENTRY') {
                        ...
                        await db.query(
                            `INSERT INTO deliveries (company_id, order_id, mission_type, status, pickup_location, drop_location, package_details, delivery_date, mode)
                             VALUES (?, ?, 'Delivery', 'pending', ?, ?, ?, ?, ?)`,
                            [order.company_id, id, order.pickup_location || 'Warehouse / HQ', dropAddr, JSON.stringify(parsedItems), orderDay, transportMode]
                        );
                    } else if (

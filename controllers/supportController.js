...
exports.createTicket = async (req, res) => {
    try {
        ...
        // Notify the submitting client/customer so they see a real-time confirmation
        await createNotification({
            companyId,
            userId: req.user.id,
            type: 'info',
            title: 'Support Ticket Opened',
            message: `Your ticket "${subject}" (#${result.insertId}) has been received. Priority: ${priority || 'medium'}.`,
...
exports.updateTicketStatus = async (req, res) => {
    try {
        ...
        // Notify the ticket submitter of the status change in real time
        if (status) {
            const [ticketRow] = await db.query('SELECT submitted_by, company_id, subject FROM support_tickets WHERE id = ?', [req.params.id]);
            ...
            if (ticket?.submitted_by) {
                await createNotification({
                    ...
                    message: `Your ticket "${ticket.subject || '#' + req.params.id}" status is now: ${status.replace('_', ' ')}.`,
                    link: '/dashboard/client-support'
                });
            }
        }
...
const PRIORITY_MAP = { 'low': 'low', 'medium': 'medium', 'high': 'high', 'urgent': 'urgent', 'immediate': 'urgent' };

...
async function getDeliveryColumnSet() {
    // MySQL compatible column introspection
    const [rows] = await db.query(`SELECT column_name FROM information_schema.columns WHERE table_name = 'deliveries' AND table_schema = DATABASE()`);
...
exports.getDeliveries = async (req, res) => {
    try {
        ...
        if (roleNorm === 'customer') {
            ...
            const resolved = companyId || 1;
            if (resolved === 1) {
                query += ` AND (d.company_id = 1 OR d.company_id IS NULL)`;
            } else {
...
exports.createDelivery = async (req, res) => {
    try {
        ...
        const insertCandidate = {
            ...
            route_distance: (route_distance != null && route_distance !== '') ? parseFloat(route_distance) : null,
            staff_pay_rate: (req.body.staff_pay_rate != null && req.body.staff_pay_rate !== '') ? parseFloat(req.body.staff_pay_rate) : null,
...
exports.updateDeliveryStatus = async (req, res) => {
    try {
        console.log('updateDeliveryStatus called for id:', req.params.id, 'with body:', req.body);
        const { status, vehicle_id, signature, driver_name, plate_number, assigned_driver, passenger_info, delivery_instructions, delivery_fee, route_distance, staff_pay_rate, mode } = req.body;
        const deliveryColumns = await getDeliveryColumnSet();
        const sets = [];
        const values = [];
        ...
        if (delivery_fee !== undefined && delivery_fee !== null && delivery_fee !== '') {
            ...
            values.push(parseFloat(delivery_fee));
        }
        if (route_distance !== undefined && deliveryColumns.has('route_distance')) {
            sets.push('route_distance = ?');
            values.push((route_distance !== null && route_distance !== '') ? parseFloat(route_distance) : null);
        }
        if (staff_pay_rate !== undefined && deliveryColumns.has('staff_pay_rate')) {
            sets.push('staff_pay_rate = ?');
            values.push((staff_pay_rate !== null && staff_pay_rate !== '') ? parseFloat(staff_pay_rate) : null);
        }
        if (mode !== undefined && deliveryColumns.has('mode')) {
            sets.push('mode = ?');
            values.push(mode);
        }
        ...
        values.push(req.params.id, ...cs.params);
        console.log(`Executing UPDATE deliveries SET ${sets.join(', ')} WHERE id = ?${cs.clause} with values:`, values);
        const [result] = await db.query(`UPDATE deliveries SET ${sets.join(', ')} WHERE id = ?${cs.clause}`, values);
        if (!result || result.affectedRows === 0) {
            console.log('Delivery not found or not permitted to update. Result:', result);

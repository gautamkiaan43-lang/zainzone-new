...
exports.getAll = async (req, res) => {
    try {
        ...
        // HQ admins see all users in their company, Super Admins see ALL, non-admins see ONLY themselves
        let cf;
        if (isSuperAdmin) {
            cf = { clause: '', params: [] };
        } else if ((roleNorm === 'admin' || roleNorm === 'manager') && isHQ) {
            // HQ admin sees ALL users in the system (same company or no company) so the
            // Deliveries → Assign Personnel dropdown shows all assignable staff
            cf = { clause: '', params: [] };
